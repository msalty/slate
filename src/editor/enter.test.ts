/**
 * Enter, where the markup it would split is invisible.
 *
 * Same fixture notation as format.test.ts: `‸` is the caret. These check where
 * the caret is *taken* before the newline goes in, because that is the whole of
 * the fix — the newline itself is still the markdown command's job.
 */

import { describe, expect, it } from 'vitest'
import { EditorState } from '@codemirror/state'
import { markdown, markdownLanguage } from '@codemirror/lang-markdown'
import { breakAbove, planEnter } from './enter'
